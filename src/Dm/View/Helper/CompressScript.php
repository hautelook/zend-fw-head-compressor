    ...
    extends Zend_View_Helper_HeadScript
{
     ...
     * @var string
     */
    protected $_defaultCacheDir = '/js/cache/';
    ...
    public function toString($indent = null)
    {
        ...
        foreach ($headScript as $item) {
            ...
            if (!$this->getProcessor()->isCachable($item)) {
                ...
                if(!empty($cache)) {
                    $items[] = $this->itemToString($this->_getCompiledItem(), $indent, $escapeStart, $escapeEnd);
    ...
    protected function _getCompiledItem()
    {
        ...
        if (!file_exists($path)) {
            ...
            foreach ($fileProcessor->getCache() as $js) {
                if(is_array($js)) {
                    $jsContent .= "/*** " . $js['path'] . " ***/\n\n" . file_get_contents($fileProcessor->getFilePath(($js['path']))) . ";\n\n";
                } else {
                    $jsContent .= "/*** Inline Source ***/\n\n$js;\n\n";
                }
            }

            // Write js content to file
            if (file_put_contents($path, $jsContent) !== false) {
                chmod($path, 0664);
            }
        }
