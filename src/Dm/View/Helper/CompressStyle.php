...
set_include_path(implode(PATH_SEPARATOR, array(
    realpath(APPLICATION_PATH . '/../lib/Tools/'),
    realpath(APPLICATION_PATH . '/../ilb/Tools/Minify/'),
    get_include_path(),
)));
    ...
    extends Zend_View_Helper_HeadLink
{
     ...
     * @var string
     */
    protected $_defaultCacheDir = '/css/cache/';
    ...
    public function toString($indent = null)
    {
        ...
        foreach ($headLink as $item) {
            ...
            if (!$this->getProcessor()->isCachable($item)) {
                $cache = $this->getProcessor()->getCache();
                if(!empty($cache)) {
                    $items[] = $this->itemToString($this->_getCompiledItem());
                    $this->getProcessor()->clearCache();
                }
    ...
    protected function _getCompiledItem()
    {
        ...
        if (!file_exists($path)) {
            ...
            foreach ($fileProcessor->getCache() as $css) {
                $content = file_get_contents($fileProcessor->getFilePath(($css['path'])));
                $cssContent .= "/*** " . $css['path'] . " ***/\n\n$content\n\n";
            }
            
            // Write css content to cache file
            if (file_put_contents($path, $cssContent) !== false) {
                chmod($path, 0664);
            }
        }
